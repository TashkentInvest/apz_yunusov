...
@section('content')
    <style>
        ...
        .quarter-item {
            @apply bg-white border-2 border-gray-200 rounded-xl p-6 hover:shadow-lg transition-all cursor-pointer;
        }
        ...
        .initial-payment-card {
            @apply bg-white border-2 border-purple-300 rounded-xl p-6 hover:shadow-lg transition-all;
        }

        .initial-payment-card.completed {
            @apply border-purple-500 bg-purple-50;
        }

        .initial-payment-card.partial {
            @apply border-purple-400 bg-purple-25;
        }

        .initial-payment-card.pending {
            @apply border-purple-300 bg-white;
        }
                        ...
                        (function() {
                            ...
                            form.addEventListener('submit', function(e) {
                                ...
                                if (confirm('Shartnoma holatini "' + statusName + '" ga o\'zgartirishni tasdiqlaysizmi?')) {
                                    ...
                                    saveBtn.innerHTML =
                                        '<svg class="animate-spin h-4 w-4 inline-block mr-1" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg> Saqlanmoqda...';
    ...
    calculateFromPercent();
        ...
        <script>
            ...
            function showInfoMessage(message) {
                ...
                alert.className =
                    'fixed top-4 right-4 bg-blue-500 text-white px-6 py-3 rounded-lg shadow-lg z-50 animate-fade-in';
                alert.innerHTML = `
            ...
            <div class="bg-white rounded-2xl shadow-lg border govt-card">
                <div class="border-b border-gray-200 p-6 flex items-center justify-between">
                    ...
                    <div class="flex space-x-3">
                        @if ($paymentData['contract']['payment_type'] === 'installment')
                            <a href="{{ route('contracts.create-schedule', $paymentData['contract']['id']) }}"
                        ...
                        @endif
                        <a href="{{ route('contracts.amendments.create', $paymentData['contract']['id']) }}"
                ...
                <div class="p-8">
                    ...
                    @php
                        $showOverdue = false;
                        $hasCompletionDate =
                            isset($paymentData['contract']['completion_date']) &&
                            !empty($paymentData['contract']['completion_date']);

                        // Only show overdue if there's actually unpaid debt
$totalPaid = floatval(
    str_replace([' so\'m', ' '], '', $paymentData['summary_cards']['total_paid_formatted']),
                        );
                        $totalPlan = floatval(
                            str_replace([' so\'m', ' '], '', $paymentData['summary_cards']['total_plan_formatted']),
                        );
                        $hasUnpaidBalance = $totalPaid < $totalPlan;

                        if (
                            $paymentData['contract']['payment_type'] === 'full' &&
                            $hasCompletionDate &&
                            $hasUnpaidBalance
                        ) {
                        ...
                        $gridCols =
                            $paymentData['contract']['payment_type'] === 'installment' ? 5 : ($showOverdue ? 4 : 3);
                    @endphp
                    <div class="grid grid-cols-1 md:grid-cols-{{ $gridCols }} gap-6 mb-8">
                        <div class="info-gradient rounded-xl p-6 text-center">
                            ...
                            <p class="text-2xl font-bold text-blue-900">
                                {{ $paymentData['summary_cards']['total_plan_formatted'] }}</p>
                        ...
                        <div class="success-gradient rounded-xl p-6 text-center">
                            ...
                            <p class="text-2xl font-bold text-green-900">
                                {{ $paymentData['summary_cards']['total_paid_formatted'] }}</p>
                        ...
                        @if ($paymentData['contract']['payment_type'] === 'installment')
                            <div class="initial-gradient rounded-xl p-6 text-center">
                                ...
                                <p class="text-2xl font-bold text-purple-900">
                                    {{ $paymentData['summary_cards']['initial_payment_paid_formatted'] }}</p>
                            ...
                            <div class="success-gradient rounded-xl p-6 text-center">
                                ...
                                <p class="text-2xl font-bold text-green-900">
                                    {{ $paymentData['summary_cards']['quarterly_paid_formatted'] }}</p>
                                <div class="mt-2 text-xs text-green-600">
                        ...
                        <div class="warning-gradient rounded-xl p-6 text-center">
                            ...
                            <p class="text-2xl font-bold text-yellow-900">
                                {{ $paymentData['summary_cards']['current_debt_formatted'] }}</p>
                        ...
                        <div class="danger-gradient rounded-xl p-6 text-center">
                            ...
                            <p class="text-2xl font-bold text-red-900">
                                {{ $paymentData['summary_cards']['overdue_debt_formatted'] }}</p>
                            <div class="mt-2 text-xs text-red-600">
                                @if ($paymentData['contract']['payment_type'] === 'full')
                                    @if ($hasCompletionDate)
                                        Muddat:
                                        {{ \Carbon\Carbon::parse($paymentData['contract']['completion_date'])->format('d.m.Y') }}
                                    @else
                                        Muddat o'tgan
                                    @endif
                                @else
                        ...
                        <div class="relative top-20 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-white">
                            <div class="mt-3">
                                ...
                                <form id="editPaymentForm" class="space-y-4">
                                    ...
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-2">To'lov summasi (so'm)
                                            *</label>
                                        <input type="number" id="editPaymentAmount" name="payment_amount"
                                    ...
                                    <div>
                                        ...
                                        <input type="text" id="editPaymentNumber" name="payment_number"
                                            maxlength="50"
                                            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500">
                                    </div>
                    ...
                    @if (
                        $paymentData['contract']['payment_type'] === 'installment' &&
                            isset($paymentData['initial_payments']) &&
                            !is_null($paymentData['initial_payments']))
                        <div class="mb-8">
                            ...
                            <div class="initial-payment-card {{ $paymentData['initial_payments']['status_class'] }}">
                                <div class="flex justify-between items-center mb-4">
                                    ...
                                    <span
                                        class="status-badge status-{{ $paymentData['initial_payments']['status_class'] }}">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                                    <div class="text-center">
                                        ...
                                        <div class="text-xl font-bold text-purple-900">
                                            {{ $paymentData['initial_payments']['plan_amount_formatted'] }}</div>
                                    </div>
                                    <div class="text-center">
                                        <div class="text-sm text-purple-700">To'langan</div>
                                        <div class="text-xl font-bold text-green-700">
                                            {{ $paymentData['initial_payments']['total_paid_formatted'] }}</div>
                                    </div>
                                    <div class="text-center">
                                        <div class="text-sm text-purple-700">Qolgan</div>
                                        <div class="text-xl font-bold text-red-700">
                                            {{ $paymentData['initial_payments']['remaining_formatted'] }}</div>
                                    </div>
                                    <div class="text-center">
                                        <div class="text-sm text-purple-700">Foiz</div>
                                        <div class="text-xl font-bold text-purple-900">
                                            {{ $paymentData['initial_payments']['payment_percent'] }}%</div>
                                    </div>
                                </div>

                                <div class="w-full bg-purple-200 rounded-full h-3 mb-4">
                                    <div class="bg-purple-600 h-3 rounded-full transition-all"
                                        style="width: {{ min(100, $paymentData['initial_payments']['payment_percent']) }}%">
                                    </div>
                                </div>
                                                    ...
                                                    <div class="flex justify-between items-start">
                                                        <div>
                                                            <div class="font-medium text-purple-900">
                                                                {{ $payment['amount_formatted'] }}</div>
                                                            <div class="text-sm text-purple-600">{{ $payment['date'] }}
                                                            </div>
                        ...
                        <div class="mb-8 bg-gray-50 rounded-2xl p-6">
                            <div class="flex items-center justify-between mb-6">
                                ...
                                <div class="flex space-x-6 text-sm">
                                    <div class="text-center">
                                        ...
                                        <div class="font-bold text-blue-600">{{ $yearData['totals']['plan_formatted'] }}
                                        </div>
                                    </div>
                                    <div class="text-center">
                                        <div class="text-gray-600">To'langan</div>
                                        <div class="font-bold text-green-600">{{ $yearData['totals']['paid_formatted'] }}
                                        </div>
                                    </div>
                                    <div class="text-center">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                @foreach ($yearData['quarters'] as $quarter => $quarterData)
                                    <div class="quarter-item {{ $quarterData['status_class'] }}"
                                        onclick="openQuarterDetails({{ $year }}, {{ $quarter }})">
                                        <div class="flex justify-between items-center mb-4">
                                            ...
                                            <div class="flex items-center space-x-2">
                                                ...
                                                @if ($quarterData['is_amendment_based'])
                                                    <span class="text-xs bg-purple-100 text-purple-800 px-2 py-1 rounded"
                                                        title="Qo'shimcha kelishuv asosida">QK</span>
                                                @endif
                                            </div>
                                        </div>
                                        ...
                                        <div class="space-y-3">
                                            <div class="flex justify-between">
                                                ...
                                                <span
                                                    class="font-semibold">{{ $quarterData['plan_amount_formatted'] }}</span>
                                            </div>
                                            <div class="flex justify-between">
                                                <span class="text-gray-600">To'langan:</span>
                                                <span
                                                    class="font-semibold text-green-600">{{ $quarterData['fact_total_formatted'] }}</span>
                                            </div>
                                            @if ($quarterData['debt'] > 0)
                                                <div class="flex justify-between">
                                                    <span class="text-gray-600">Qarz:</span>
                                                    <span
                                                        class="font-semibold {{ $quarterData['is_overdue'] ? 'text-red-600' : 'text-yellow-600' }}">
                                        ...
                                        <div class="mt-4">
                                            ...
                                            <div class="mt-3 flex justify-between items-center">
                                                ...
                                                <div class="flex space-x-1">
                                                    <button
                                                        onclick="addQuarterPayment({{ $year }}, {{ $quarter }})"
                                                    ...
                                                    @if (count($quarterData['payments']) > 0)
                                                        <button
                                                            onclick="showQuarterPayments({{ $year }}, {{ $quarter }})"
                                ...
                                @foreach (array_slice($paymentData['payment_history']['payments'], 0, 9) as $payment)
                                    <div class="payment-card {{ $payment['is_initial_payment'] ? 'border-purple-200' : 'border-gray-200' }}"
                                        ...
                                        <div class="flex justify-between items-start mb-2">
                                            ...
                                            <div
                                                class="text-xs {{ $payment['is_initial_payment'] ? 'bg-purple-100 text-purple-600' : 'bg-gray-100 text-gray-600' }} px-2 py-1 rounded">
                                        ...
                                        <div class="flex justify-between items-start">
                                            ...
                                            <div class="flex items-center space-x-2">
                                                ...
                                                @if (!$amendment['is_approved'])
                                                    ...
                                                    <button
                                                        onclick="deleteAmendment({{ $amendment['id'] }}, '{{ $amendment['amendment_number'] }}')"
                                                        class="p-1 bg-red-100 text-red-600 rounded hover:bg-red-200"
                                                        title="O'chirish">
            ...
            <div class="mt-3">
                ...
                <form id="addPaymentForm" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="number" id="modalPaymentAmount" name="payment_amount" step="0.01"
                            min="0.01" required
    ...
    <script>
            ...
            if (type === 'initial') {
                ...
                if (remainingAmount > 0) {
                    ...
                    helperText.textContent =
                        `Qolgan boshlang'ich to'lov: ${new Intl.NumberFormat('uz-UZ').format(remainingAmount)} so'm`;
            ...
            if (amountInput) {
                amountInput.addEventListener('input', function() {
                    ...
                    if (maxAmount && currentAmount > maxAmount) {
                        ...
                        error.textContent =
                            `Summa ${new Intl.NumberFormat('uz-UZ').format(maxAmount)} so'mdan oshmasligi kerak`;
        ...
        function submitAddPayment() {
            ...
            if (maxAmount && currentAmount > maxAmount) {
                showErrorMessage(
                    `To'lov summasi ${new Intl.NumberFormat('uz-UZ').format(maxAmount)} so'mdan oshmasligi kerak`);
                return;
            }
        ...
        function showSuccessMessage(message) {
            const alert = document.createElement('div');
            alert.className =
                'fixed top-4 right-4 bg-green-500 text-white px-6 py-3 rounded-lg shadow-lg z-[60] transform transition-all duration-300 max-w-md';
            alert.innerHTML = `
        <div class="flex items-center">
            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    `;
        ...
        function showErrorMessage(message) {
            const alert = document.createElement('div');
            alert.className =
                'fixed top-4 right-4 bg-red-500 text-white px-6 py-3 rounded-lg shadow-lg z-[60] transform transition-all duration-300 max-w-md';
            alert.innerHTML = `
        <div class="flex items-center">
            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
